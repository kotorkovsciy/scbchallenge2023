...
class parseHh:
    ...
    def __page_pag(self, soup):
        return soup.find_all("span", {"class": "pager-item-not-in-short-range"})[
            -1
        ].text
    ...
    def parse_single_resume(self, serp):
        ...
        if (
            serp.find("div", {"class": "bloko-text bloko-text_large bloko-text_strong"})
            is not None
        ):
            salary = serp.find(
                "div", {"class": "bloko-text bloko-text_large bloko-text_strong"}
            ).text
        else:
            salary = 0
        ...
        if serp.find("span", {"data-qa": "resume-serp__resume-age"}) is not None:
            age = (
                serp.find("span", {"data-qa": "resume-serp__resume-age"})
                .find("span")
                .text
            )
        else:
        ...
        if (
            serp.find("div", {"data-qa": "resume-serp__resume-excpirience-sum"})
            is not None
        ):
            excpirience_sum = (
                serp.find("div", {"data-qa": "resume-serp__resume-excpirience-sum"})
        ...
        if serp.find("button", {"data-qa": "last-experience-link"}) is not None:
            last_experience_link = serp.find(
                "button", {"data-qa": "last-experience-link"}
            ).text
        else:
            last_experience_link = 0

        last_update = (
            serp.find("div", {"class": "bloko-text bloko-text_tertiary"})
            .find_all("span")[2]
            .text
        )
